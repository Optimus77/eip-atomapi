...
public class EipDaoService {
    ...
    public Eip allocateEip(EipAllocateParam eipConfig, EipPool eip, String portId) throws KeycloakTokenException {
        ...
        log.info("User:{} success allocate eip:{}", userId, eipMo.getId());
        return eipMo;
    }
...
//        Eip eipEntity = eipRepository.findByEipId(eipid);
        ...
        if(eipEntity.getBillType().equals(HsConstants.MONTHLY)){
            if(CommonUtil.isOwner(eipEntity.getUserId())){
                log.error("Owner can not delete monthly eip, user:{},eip:{}", eipEntity.getUserId(), eipEntity.getEipAddress());
...
//        Eip eipEntity = eipRepository.findByEipId(eipid);
    ...
    public MethodReturn associateInstanceWithEip(String eipid, String serverId, String instanceType, String portId, String fip){
        ...
        Eip eip = null;
        Optional<Eip> eipOptional = eipRepository.findById(eipid);
